...
#tag WindowCode
	...
	#tag Event
		Sub ToolbarPressed(button As iOSToolButton)
		  ...
		  if button.Caption = "Animate" then
		    ImageView1.AppleView.TransitionWithAlpha (0.2, AppleView.UIVIewAnimationTransition.FlipFromRight, AppleViewAnimationOption.OptionRepeatAndReverse, 2.5, AppleView.UIVIewAnimationCurve.EaseInEaseOut)
		    ...
		    ImageView1.AppleView.AnimateTransform myTransform, myoption, 3, AppleView.UIVIewAnimationCurve.Linear
		    ImageView1.AppleView.TranslatesAutoresizingMaskIntoConstraints = true
		    
		    Animate
		  else
		    dim help as new InfoView ("Block animations are property animations on some of the properties of a UIView, so you can animate about every visible object on the screen. You can even attach them to a iOSView's AppleView object like I did here with the backgroundcolor property." + endofline + _
		    "Block animations get their name from the block that is used as a delegate surrogate which sends you activation and end events. In many cases you will find the CAAnimations on the AppleCALayer more powerful (see the demo view). "+ _
	...
	#tag Method, Flags = &h0
		Sub Animate()
		  ...
		  table1.AppleView.AnimateTransform myTransform, myoption, 5, AppleView.UIVIewAnimationCurve.Linear, 1
