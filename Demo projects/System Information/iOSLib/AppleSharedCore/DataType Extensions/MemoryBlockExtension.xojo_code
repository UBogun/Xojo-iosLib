...
Protected Module MemoryBlockExtension
	#tag Method, Flags = &h0
		Function CArrayMBToUIntegerArray(extends CStringArrayMB as xojo.Core.MemoryBlock) As uinteger()
		  dim count as uinteger = CStringArrayMB.Size/IntegerSize
		  dim result() as uinteger
		  for q as uinteger = 0 to count -1
		    #if target64bit
		      result.Append CStringArrayMB.UInt64Value(q*IntegerSize)
		    #elseif Target32Bit
		      result.Append CStringArrayMB.UInt32Value(q*IntegerSize)
		    #endif
		  next
		  Return result
		End Function
	...
	#tag Method, Flags = &h0
		Function fromDoubleCArray(extends mb as memoryblock, optional count as integer) As Double()
		  dim result() as double
		  count = mb.Size / 8 - 2
		  for q as UInteger = 0 to count
		    result.Append mb.DoubleValue(q*8)
		  next
		  return result
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function fromSingleCArray(extends mb as memoryblock, optional count as integer) As double()
		  dim result() as double
		  if count = 0 then count = mb.Size / 4
		  for q as UInteger = 0 to count -1
		    result.Append mb.SingleValue(q*4)
		  next
		  return result
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function toDoubleCArray(Doubles() as double) As MutableMemoryblock
		  dim count as integer = doubles.Ubound
		  dim mb as new MutableMemoryBlock ((count + 2) * 8)
		  for q as UInteger = 0 to count
		    mb.DoubleValue(q * 8) = doubles(q)
		  next
		  return mb
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function toSingleCArray(Doubles() as double) As MutableMemoryblock
		  dim count as integer = doubles.Ubound
		  dim mb as new MutableMemoryBlock ((count + 2) * 4)
		  for q as UInteger = 0 to count
		    mb.SingleValue(q * 4) = doubles(q)
		  next
		  return mb
		End Function
	#tag EndMethod
