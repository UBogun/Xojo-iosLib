		    ...
		    for each CodeP as uint32 in char.Codepoints
		      ...
		      if objcTypeDict.HasKey (CodeP) then
		        ...
		        case 123 // {
		          ...
		          dim nextEnd as uinteger = mytext.IndexOf(q+1, "}")
		          if nextend > -1 then
		            if (nextQuotePos > -1  and nextend < nextQuotePos) or nextQuotePos = -1 then
		              result = result + mytext.mid (q+1, nextend -1 -q)+"}, "
		              q=nextEnd
		            elseif nextQuotePos > -1 then
