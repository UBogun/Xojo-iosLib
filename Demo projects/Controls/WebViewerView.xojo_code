...
#tag WindowCode
	...
	#tag Event
		Sub ToolbarPressed(button As iOSToolButton)
		  dim help as new InfoView ("Does this look exactly like the NotificationsView? Yes, but its different – the viewer is not a Xojo iOSHTMLViewer but a custom control named iOSLibHTMLViewer. This owns a delegate that sends four different events to the control: "+eol+ _
		  "ShouldStart (with a Enumeration value telling you what Webkit event caused the update. The delegate returns true which means »load« but you could extend the implementation on your demands."+eol+ _
		  "Did Start"+eol +"Did Finish"+eol+"Failed"+eol +"Shouldstart carries the request too and Failed a NSError object."+eol+eol+ _
