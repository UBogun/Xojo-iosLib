#tag Class
Protected Class AppleSecurityTrust
Inherits AppleCFObject
	...
	#tag Method, Flags = &h21
		Private Sub AsyncBlock(Sectrust as ptr, Result as TrustResultType)
		  if me <> nil then RaiseEvent Evaluated (new AppleSecurityTrust (Sectrust), result)
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		 Shared Function MakefromCFTypeRef(aCFTypeRef as ptr) As AppleSecurityTrust
		  return if (aCFTypeRef = nil, nil, new AppleSecurityTrust(aCFTypeRef))
		End Function
	#tag EndMethod
	...
	#tag Hook, Flags = &h0
		Event Evaluated(Trust as AppleSecurityTrust, Result as TrustResultType)
	#tag EndHook
