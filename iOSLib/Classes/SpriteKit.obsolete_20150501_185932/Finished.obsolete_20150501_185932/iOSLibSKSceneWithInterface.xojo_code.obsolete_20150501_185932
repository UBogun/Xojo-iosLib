#tag Class
Protected Class iOSLibSKSceneWithInterface
Inherits iOSLibSKScene
Implements iOSLibEventForwarder
	...
	#tag Method, Flags = &h1000
		Sub Constructor(observer As ioslibskviewer)
	...
	#tag Method, Flags = &h1000
		Sub Constructor(asize as NSSize, observer As iosLibEventReceiver)
		...
		Sub Destructor()
		  if mhasownership then
		    ...
		    if receivers <> nil then
		      if Receivers.HasKey(id) then RemoveObserver Receivers.Value(id)
		    end if
	...
	#tag Method, Flags = &h21
		Private Shared Function GetView(PId as Ptr) As iOSLibSKViewer
		  if Receivers <> nil then
		    if Receivers.HasKey (pid) then
		      dim WR as WeakRef = Receivers.Value(pid)
		      if wr.Value <> nil then return ioslibskviewer(wr.Value)
		    end if
		  end if
	...
	#tag Method, Flags = &h0
		 Shared Function MakefromPtr(aPtr as Ptr) As iOSLibSKSceneWithInterface
		  return if (aptr = nil, nil, new iOSLibSKSceneWithInterface (aptr))
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub NotifyObservers(EventProperties As iOSLibArray)
		  ...
		  // dim observer as iOSLibEventReceiver = Receivers.Value(id)
			...
			Get
			  ...
			  if iOSLibSKView.SpriteKitEnabled then
			    if targetID = Nil then
			      ...
			      #endif
			      targetID = BuildTargetClass ("SKScene", "iOSLibSKViewScene",methods)
			    end if
			  end if
			  Return targetID
			End Get
