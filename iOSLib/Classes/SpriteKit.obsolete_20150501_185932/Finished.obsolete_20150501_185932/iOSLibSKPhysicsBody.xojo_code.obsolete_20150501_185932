#tag Class
Protected Class iOSLibSKPhysicsBody
Inherits iosLibObject
	...
	#tag Method, Flags = &h1000
		Sub Constructor(BodyArray as iOSLibArray)
	...
	#tag Method, Flags = &h1000
		Sub Constructor(Polygon as iOSLibCGPath)
	...
	#tag Method, Flags = &h1000
		Sub Constructor(Node As iOSLibSKNode)
		  Constructor (node.Frame.Size_)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h1000
		Sub Constructor(Texture As iOSLibSKTexture)
	...
	#tag Method, Flags = &h1000
		Sub Constructor(Texture As iOSLibSKTexture, AlphaTreshold as single)
		  Constructor (Texture, AlphaTreshold, Texture.Size)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h1000
		Sub Constructor(Texture As iOSLibSKTexture, AlphaTreshold as single, size as NSSize)
		  #if Target64Bit
	...
	#tag Method, Flags = &h1000
		 Shared Function EdgeChain(aPath as iOSLibCGPath) As iOSLibSKPhysicsBody
		  ...
		  dim result as new iOSLibSKPhysicsBody (bodyWithEdgeChainFromPath (ClassPtr, aPath.CFTypeRef))
	...
	#tag Method, Flags = &h1000
		 Shared Function EdgeLoop(aPath as iOSLibCGPath) As iOSLibSKPhysicsBody
		  declare Function bodyWithEdgeLoopFromPath lib SpriteKit selector "bodyWithEdgeLoopFromPath:" (id as ptr,  aPath as Ptr) as ptr
		  dim result as  new iOSLibSKPhysicsBody (bodyWithEdgeLoopFromPath (ClassPtr, aPath.CFTypeRef))
		  result.RetainClassObject
		  return result
		  // Calling the overridden superclass constructor.
		  // Note that this may need modifications if there are multiple constructor choices.
	...
	#tag Method, Flags = &h1000
		 Shared Function EdgeLoop(P1 as NSPoint, P2 as NSPoint) As iOSLibSKPhysicsBody
		  dim result as iOSLibSKPhysicsBody
		  #if Target64Bit
		    ...
		    result = new iOSLibSKPhysicsBody (bodyWithEdgeFromPoint (ClassPtr, P1, P2))
		  #elseif Target32Bit
		    ...
		    result = new iOSLibSKPhysicsBody (bodyWithEdgeFromPoint (ClassPtr, P1.toNSPoint32, P2.toNSPoint32))
		  #endif
		  result.RetainClassObject
		  return result
		  // Calling the overridden superclass constructor.
	...
	#tag Method, Flags = &h1000
		 Shared Function EdgeLoop(aRect as NSRect) As iOSLibSKPhysicsBody
		  dim result as iOSLibSKPhysicsBody
		  #if Target64Bit
		    ...
		    result = new iOSLibSKPhysicsBody (bodyWithEdgeLoopFromRect (ClassPtr, arect))
		  #elseif Target32Bit
		    ...
		    result = new iOSLibSKPhysicsBody (bodyWithEdgeLoopFromRect (ClassPtr, arect.toNSRect32))
		  #endif
		  result.RetainClassObject
		  return result
	...
	#tag Method, Flags = &h0
		 Shared Function MakeFromPtr(aPtr as Ptr) As iOSLibSKPhysicsBody
		  Return if (aptr = NIL, NIL, new iOSLibSKPhysicsBody (aptr))
		End Function
	#tag EndMethod
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  ...
			  return iOSLibArray.MakeFromPtr (allContactedBodies(id))
		...
		#tag Setter
			...
			End Set
		#tag EndSetter
		AllContactedBodies As iOSLibArray
	#tag EndComputedProperty
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  ...
			  return iOSLibArray.MakeFromPtr (joints(id))
			  
			End Get
		#tag EndGetter
		Joints As iOSLibArray
	#tag EndComputedProperty
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  ...
			  return iOSLibSKNode.MakeFromPtr (node(id))
			  
			End Get
		#tag EndGetter
		Node As iOSLibSKNode
	#tag EndComputedProperty
