...
Inherits AppleObject
	...
	#tag Method, Flags = &h0
		Sub Constructor(fileURL as AppleURL, settings as appledictionary, commonFormat as AVAudioCommonFormat, Interleaved as boolean, error as AppleError= nil)
		  // Calling the overridden superclass constructor.
		  // Note that this may need modifications if there are multiple constructor choices.
		  // Possible constructor calls:
		  // Constructor() -- From AppleObject
	...
	#tag Method, Flags = &h0
		Sub Constructor(fileURL as AppleURL, commonFormat as AVAudioCommonFormat, Interleaved as boolean, error as AppleError = nil)
		  // Calling the overridden superclass constructor.
		  // Note that this may need modifications if there are multiple constructor choices.
		  // Possible constructor calls:
		  // Constructor() -- From AppleObject
	...
	#tag ExternalMethod, Flags = &h21
		Private Declare Function InitForReadingCommonFormat Lib AVFoundationLibname Selector "initForReading:commonFormat:interleaved:error:" (id as ptr, fileURL as ptr, commonFormat as AVAudioCommonFormat, Interleaved as boolean, error as Ptr) As Ptr
	#tag EndExternalMethod
	...
	#tag ExternalMethod, Flags = &h21
		Private Declare Function InitForWritingCommonFormat Lib AVFoundationLibname Selector "initForWriting:settings:commonFormat:interleaved:error:" (id as ptr, fileURL as ptr, settings as ptr, commonFormat as AVAudioCommonFormat, Interleaved as boolean, error as Ptr) As Ptr
	#tag EndExternalMethod
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  static mClassPtr as ptr = FoundationFramework.NSClassFromString ("AVAudioFile")
			  return mClassPtr
			End Get
		#tag EndGetter
