...
Protected Module iOSGraphicsExtension
	...
	#tag Method, Flags = &h0
		Sub DrawPath(extends g as iosgraphics, drawingmode as CGPathDrawingMode)
	...
	#tag Method, Flags = &h0
		Function DrawTiledImage(extends g as iOSGraphics, image as iOSImage, TileSize as xojo.core.rect) As Double
		  g.SaveState // saving the current state because the context needs to be flipped for the tiledimage call
		  g.Translate (0, g.Height) // flip it
		  g.Scale (1, -1)
	...
	#tag Method, Flags = &h0
		Sub FontSubpixelQuantization(extends g as iOSGraphics, assigns value as boolean)
		  g.CGContext.AllowFontSubpixelQuantization = value
		  g.CGContext.ShouldSubpixelQuantizeFonts = value
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Function InterpolationQuality(extends g as iOSGraphics) As CGInterpolationQuality
		  return g.CGContext.InterpolationQuality
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub InterpolationQuality(extends g as iOSGraphics, assigns value as CGInterpolationQuality)
	...
	#tag Method, Flags = &h0
		Function LineCap(extends g as iosgraphics) As CGLineCap
		  return g.LineCap
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub LineCap(extends g as iosgraphics, assigns value as CGLineCap)
		  g.LineCap = value
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub SetShadow(extends g as iosgraphics, XOffset as Double, YOffset as double, Blur as double, ShadowColor as Color)
		  g.CGContext.SetShadow XOffset, YOffset, Blur, ShadowColor
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub TextDrawingMode(extends g as iOSGraphics, assigns value as CGTextDrawingMode)
